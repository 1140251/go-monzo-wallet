...
func NewTheme(fontCollection []text.FontFace, decredIcons map[string]image.Image, isDarkModeOn bool) *Theme {
	t := &Theme{
		...
		Color:    &assets.Color{},
		Icons:    &values.Icons{},
		Styles:   DefaultWidgetStyles(),
		TextSize: values.TextSize16,
	}
	t.SwitchDarkMode(isDarkModeOn, decredIcons)
	t.checkBoxCheckedIcon = values.MustIcon(widget.NewIcon(icons.ToggleCheckBox))
	t.checkBoxUncheckedIcon = values.MustIcon(widget.NewIcon(icons.ToggleCheckBoxOutlineBlank))
	t.radioCheckedIcon = values.MustIcon(widget.NewIcon(icons.ToggleRadioButtonChecked))
	t.radioUncheckedIcon = values.MustIcon(widget.NewIcon(icons.ToggleRadioButtonUnchecked))
	t.chevronUpIcon = values.MustIcon(widget.NewIcon(icons.NavigationExpandLess))
	t.chevronDownIcon = values.MustIcon(widget.NewIcon(icons.NavigationExpandMore))
	t.navMoreIcon = values.MustIcon(widget.NewIcon(icons.NavigationMoreHoriz))
	t.navigationCheckIcon = values.MustIcon(widget.NewIcon(icons.NavigationCheck))
	t.dropDownIcon = values.MustIcon(widget.NewIcon(icons.NavigationArrowDropDown))
...
func fillMax(gtx layout.Context, col color.NRGBA, radius Radius) values.D {
	cs := gtx.Constraints
